...
async function run() {
  console.log('╔══════════════════════════════════╗');
  console.log('║  Step 7: Addresses (FIXED)       ║');
  console.log('╚══════════════════════════════════╝\n');
  
  // Get first attribute for default
  const attr = await prisma.vf_attributes.findFirst({ select: { id: true } });
  if (!attr) {
    console.log('❌ No attributes exist yet. Run migrate_03_patients.js first!');
    return;
  }
  console.log(`Using default attributes_id: ${attr.id}`);
  ...
  for (const a of addresses) {
    ...
    created++;
    if (created % 500 === 0) console.log(`   📊 ${created} addresses created`);
  }
  
  console.log(`\n✅ Addresses: created=${created}, skipped=${skipped}`);
