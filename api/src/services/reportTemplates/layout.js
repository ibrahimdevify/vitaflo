...
/** "All Time" when the PDF's date range was defaulted (no dates given by the caller), otherwise the formatted range. */
function rangeLabel(startDate, endDate, isAllTime) {
  return isAllTime ? 'All Time' : `${fmtDate(startDate)} to ${fmtDate(endDate)}`;
}
