...
const syncSpirometryPlus = async (req, res) => {
  try {
    ...
    const observation = await prisma.portal_observation.create({ data: { user_id: userId, dbdate: now, is_post_bronchodilator: isPostBronchodilator === "true" || isPostBronchodilator === true, height: attributes?.height } });
