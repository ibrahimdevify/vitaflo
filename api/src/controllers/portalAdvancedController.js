...
const getSpirometryTrends = async (req, res) => {
  try {
    ...
    const data = await prisma.portal_spirometry.findMany({
      where: {
        observation: { user_id: parseInt(user_id) },
...
const getIAQTrends = async (req, res) => {
  try {
    ...
    const data = await prisma.portal_indoor_air_quality.findMany({
      where: { user_id: parseInt(user_id), dbdate: { gte: new Date(start), lte: new Date(end) } },
      orderBy: { dbdate: 'asc' },
    });
    res.json({ data, total: data.length });
  } catch (error) {
...
const getAlerts = async (req, res) => {
  try {
    ...
    const data = await prisma.portal_alert.findMany({
      where, orderBy: { created: 'desc' }, take: 100,
      include: { user: { select: { user_id: true, f_name: true, l_name: true } } },
    });
    res.json({ data });
  } catch (error) {
    res.status(500).json({ error: error.message });
