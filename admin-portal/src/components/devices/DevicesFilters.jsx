...
export default function DevicesFilters({
  ...
  const selected = filterOptions.find((o) => o.value === filterActive);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-40">
          {filterOptions.map((o) => (
            <DropdownMenuItem
              key={o.value}
              onClick={() => onFilterActiveChange(o.value)}
              className={cn(
                'cursor-pointer',
                filterActive === o.value && 'bg-surface-raised font-medium'
              )}
            >
              {o.label}
              {filterActive === o.value && (
