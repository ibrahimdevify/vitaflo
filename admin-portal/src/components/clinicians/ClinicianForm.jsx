  ...
  const onSubmit = async (data) => {
    try {
      const payload = {
        ...
        password: data.password || "Doctor@123456",
        license_no: data.license_no.trim(),
        experience: data.experience || "2 yrs",
        about_doctor: data.about_doctor || "",
        education: data.education || "",
        is_specialist: data.is_specialist || false,
        h_id_fk: 1,
      };
      ...
      console.log("Clinician payload:", payload);

      if (isEditing) {
        await cliniciansAPI.update(initialData.user_id, payload);
        toast.success("Clinician updated successfully!");
      } else {
        await cliniciansAPI.create(payload);
        toast.success("Clinician created successfully!");
      }

      if (onSuccess) onSuccess();
    } catch (err) {
      console.error("Failed to save clinician:", err);
      const errorMsg =
        err.response?.data?.error ||
        err.response?.data?.message ||
        "Failed to save clinician";
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="space-y-5 mb-6">
            <h4 className="text-subheading font-semibold text-fg">
              Basic Information
            </h4>
              ...
              <Field
                label={
                  isEditing
                    ...
                    : "Password (default: Doctor@123456)"
                }
